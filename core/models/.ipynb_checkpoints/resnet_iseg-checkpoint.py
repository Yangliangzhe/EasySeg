...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=6, zero_init_residual=False,
        ...
        mt_layers = [
            nn.Conv2d(in_channels=num_classes*2, out_channels=num_classes*2, kernel_size=3),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=num_classes*2, out_channels=32, kernel_size=3, stride=2, padding=1),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, interactive_fmaps=None):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        if interactive_fmaps is not None:
            interactive_fmaps = self.maps_transform(interactive_fmaps)
            x = x + interactive_fmaps#*1000
...
class Initializer(object):
    ...
    def __call__(self, m):
        if getattr(m, '__initialized', False):
            return
        #BN,IN,GN,SBN都的w和b都用gamma(方差：1或1为均值的方差给定的正太初始化)和beta(均值：0)
        ...
        #其他层权重为-0.07~0.07的均匀初始化，偏差做0初始化 ？？？
